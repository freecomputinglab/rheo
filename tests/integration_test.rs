...
use helpers::{
    comparison::{
        compare_html_content, compare_pdf_metadata, extract_pdf_metadata,
        validate_html_assets, validate_pdf_assets, verify_included_files_present,
    },
    discovery,
    fixtures::TestCase,
...
fn test_directory_mode() {
    let test_cases = discovery::discover_directory_tests();

    for test_case in test_cases {
        println!("Running test: {:?}", test_case);
...
fn run_test_case(test_case: &TestCase) {
    ...
    let _project = ProjectConfig::from_path(project_path, None)
    ...
    if run_html {
        ...
        if html_output.exists() {
            if update_mode {
                update_html_references(test_name, &html_output)
...
fn verify_html_output(test_name: &str, actual_dir: &PathBuf) {
    let ref_dir = PathBuf::from("tests/ref").join(test_name).join("html");
...
fn verify_pdf_output(test_name: &str, actual_dir: &PathBuf) {
    let ref_dir = PathBuf::from("tests/ref").join(test_name).join("pdf");
    ...
    validate_pdf_assets(&ref_dir, actual_dir)
        .expect("PDF asset validation failed");
